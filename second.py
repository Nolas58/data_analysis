...
# Установка индекса и преобразование данных
...
# Прогнозирование на период 2024-2030
forecasted_data = {}
...
for column in ['Soybeans', 'Sunflower Oil', 'Wheat', 'Phosphate Rock']:
    model_final = ARIMA(commodity_data[column], order=(5,1,0))
...
# Визуализация временных рядов с реальными и прогнозируемыми данными
plt.figure(figsize=(14, 8))
for column in ['Soybeans', 'Sunflower Oil', 'Wheat', 'Phosphate Rock']:
    plt.plot(commodity_data.index, commodity_data[column], label=f'Реальные данные: {column}')
    plt.plot(forecast_df_all.index, forecast_df_all[column], linestyle='--', marker='o', label=f'Прогнозируемые данные: {column}')
